 ...
 * {@link io.dbeaver.mcp.McpServerPreferences}). Optionally requires a bearer
 * token (see {@link #setAuth(BooleanSupplier, Supplier)}) and always records
 * recent client connections for display in the UI.
 */
public class McpHttpServer {
    ...
    public synchronized void start(String host, int port) {
        ...
        try {
            ...
            boundHost = host;
            boundPort = port;
            log("MCP server listening on http://" + host + ":" + port + PATH);
        } catch (IOException e) {
            httpServer = null;
    ...
    private static void log(String message) {
        System.out.println("[dbeaver-mcp] " + message);
    }
}
